  ...
  messages: [
    {
      ...
      ...(NO_IMAGE
        ? {}
        : { images: [readFileSync(IMAGE).toString("base64")] }),
    },
  ],
  format: jsonSchema,
  think: process.env.THINK === "1" ? true : false, // Qwen3-VL is a reasoning model; default OFF so tokens go to JSON, not <think>
...
if (!res.ok) {
  ...
  console.log(JSON.stringify({ model: MODEL, ok: false, stage: "http", status: res.status, error: errText.slice(0, 800) }));
  process.exit(2);
}
...
try { writeFileSync(`${slug}.raw.txt`, content); } catch {}
try { if (thinking) writeFileSync(`${slug}.thinking.txt`, thinking); } catch {}
...
const promptTokSec = ns(data.prompt_eval_duration) > 0 ? data.prompt_eval_count / (ns(data.prompt_eval_duration) / 1e9) : null;
...
let parsed = null, valid = false, validationError = null, jsonSource = null;
for (const [src, text] of [["content", content], ["thinking", thinking]]) {
  const t = (text || "").trim();
  if (!t) continue;
  try {
    const p = JSON.parse(t);
    const r = Doc.safeParse(p);
    parsed = p; jsonSource = src; valid = r.success;
